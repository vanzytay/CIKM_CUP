...
def write_to_file(results, filename, with_scores):
...
nn_pairs= []
for ps in nn_pairs_lst[:1]: #!!! skip doc2vec candidates
	nn_pairs += ps
...
def predict_by_rf(rf_model, candidates_sets, strict_mode, nn_pairs=None):
	...
	res = [(p[0],p[1],cs[p[0]][p[1]]) for p in r_ps]
	res = sorted(res, key=lambda x: x[-1],  reverse=True)
	timer.finish()

	if strict_mode:
		res = [_ for _ in res if _[2]>0.5]

	return res

results = predict_by_rf(xgb1, dev_candidates_sets, False, None)

# write_to_file(results[:215307], 'result_ordered.submit.txt')
# raise

evaluate_on_test_100k(results[:110000])
evaluate_on_test_100k(results[:200000])
# raise
...
'''
write_to_file(get_top_pair_for_submit(XGB2_ext_pairs+XGB1_results), 'result_ordered.with-inference.submit.txt', False)